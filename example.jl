...
# ╔═╡ 603f819c-fc06-11ed-2dcc-bd2316082171
using CairoMakie, PlutoUI

# ╔═╡ d24ee336-886d-45f6-9a10-747dc21ce391
md"""
Move slider to set the amplitude of `sin(x)` (red line): $(@bind A PlutoUI.Slider(1:10; show_value = true, default = 1))

Move slider to set the amplitude of `cos(x)` (blue line): $(@bind B PlutoUI.Slider(1:10; show_value = true, default = 1))
"""

# ╔═╡ f2ec5d8d-c240-49bd-9f13-a9fff463335f
x = LinRange(0, 2π, 50);

# ╔═╡ dbcf325c-405f-4d1f-aca9-9773afa929b2
let
	...
	lines!(
		x, A*sin.(x); color = :red, label = "A × sin(x)"
	)
	scatterlines!(
		x, B*cos.(x); color = :blue, label = "B × cos(x)", markercolor = :black, markersize = 10
	)
	scatter!(
		x, -cos.(x); color = :red, label = "-cos(x)", strokewidth = 1, strokecolor = :red, markersize = 5, marker = '■'
	)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "629e99703292bfe2020dc9d7ff933616006b7824"
